...
static void io_acct_start(struct bio_wrapper *biow)
{
	...
	cpu = part_stat_lock();
	part_round_stats(bdev_get_queue(biow->bio->bi_bdev), cpu, part0);
	part_stat_inc(cpu, part0, ios[rw]);
	part_stat_add(cpu, part0, sectors[rw], biow->len);
	part_inc_in_flight(bdev_get_queue(biow->bio->bi_bdev), part0, rw);
	part_stat_unlock();
...
static void io_acct_end(struct bio_wrapper *biow)
{
	...
	part_round_stats(bdev_get_queue(biow->bio->bi_bdev), cpu, part0);
	part_dec_in_flight(bdev_get_queue(biow->bio->bi_bdev), part0, rw);
	part_stat_unlock();
